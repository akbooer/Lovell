...
-- 2025.02.17   remove CJson and RapidJson references for use in LÖVE, add read() and write()
...
function _M.read(path)
  local jdata, err
  local file = lf.newFile(path, 'r')
  if file then 
--    _log("loading " .. path)
    local info = file: read()
    file: close()
    jdata, err = _M.decode(info)
  end
  return jdata, err
end

function _M.write(path, info)
  local f, err = lf.newFile(path, 'w')
--  _log ("saving " .. path)  
  if not f then return nil, err end
  info, err = _M.encode(info)
