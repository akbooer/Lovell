...
local _M = {
  ...
  VERSION = "2026.02.10",
  AUTHOR = "AK Booer",
  DESCRIPTION = "THREAD watches or reloads FITS file folders",
}

local love = _G.love

local reader = love.thread.getChannel "reader"
...
-- 2026.02.10  renames "reader", combining "watcher" and "reloader" threads
...
--    table.sort(dir)
    ...
    for _, file in ipairs(dir) do
      if file: lower() : match "%.fi?ts?$" and not files[file] then
        files[file] = true
...
repeat
  ...
  local command, newfolder = get_message()
  execute[command] (newfolder or folder)
