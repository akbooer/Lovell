...
local _M = require "guillaume.objects" .GUIobject()
  ...
  _M.NAME = ...
  _M.VERSION = "2024.11.23"
  _M.DESCRIPTION = "test GÜI"
...
local dsos = require "session" .dsos
...
--    local Name, RA, Dec, Con, OT, Mag, Diam, Other  
    local x = 20
    local y = 30 + i * h
    local position = Ndso*(1 - scroll.value)
    local fraction = math.floor(h * (position % 1))
    local cols = dsos[math.min(Ndso, i + math.floor(position))]
    for j = 1,#cols do
      local col = cols[j]
